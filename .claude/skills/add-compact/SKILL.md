...
> **Note:** `upstream` is the remote pointing to `qwibitai/pepper`. If using a different remote name, substitute accordingly.
...
1. Start Pepper in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/pepper-test
cd /tmp/pepper-test
...
- **No config file.** Pepper's philosophy is customization through code changes, not configuration sprawl.
