...
This skill adds Telegram support to Pepper, then walks through interactive setup.
...
```bash
git remote add telegram https://github.com/qwibitai/pepper-telegram.git
```
...
- Check bot is started: `tail -f logs/pepper.log`
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.pepper` (macOS) or `npm run build && systemctl --user restart pepper` (Linux)
