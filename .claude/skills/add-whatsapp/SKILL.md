...
This skill adds WhatsApp support to Pepper. It installs the WhatsApp channel code, dependencies, and guides through authentication, registration, and configuration.
...
```bash
git remote add whatsapp https://github.com/qwibitai/pepper-whatsapp.git
```
...
This happens when two instances connect with the same credentials. Ensure only one Pepper process is running:
...
4. Service is running: `launchctl list | grep pepper` (macOS) or `systemctl --user status pepper` (Linux)
5. Logs: `tail -50 logs/pepper.log`
...
4. Rebuild and restart: `npm run build && launchctl kickstart -k gui/$(id -u)/com.pepper` (macOS) or `npm run build && systemctl --user restart pepper` (Linux)
