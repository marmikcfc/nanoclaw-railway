---
name: x-integration
description: X (Twitter) integration for Pepper. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** Pepper v1.0.0. Directory structure may change in future versions.
...
1. **Pepper is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep pepper (macOS) or systemctl --user status pepper (Linux)
```
...
| `PEPPER_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/pepper.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into Pepper, make the following modifications:
...
All paths below are relative to project root (`PEPPER_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep pepper  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status pepper
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/pepper.log | tail -20

# Script errors
grep -i "error\|failed" logs/pepper.log | tail -20
```
...
docker run pepper-agent ls -la /app/src/skills/
```
