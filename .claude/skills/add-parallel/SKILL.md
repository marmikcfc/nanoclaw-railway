...
Adds Parallel AI MCP integration to Pepper for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Pepper scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Pepper already set up and running
...
Add Parallel AI MCP servers after the pepper server:
```typescript
const mcpServers: Record<string, any> = {
  pepper: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__pepper__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__pepper__send_message
   4. Use mcp__pepper__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo pepper-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep pepper  # macOS
...
- Check task runs: `tail -f logs/pepper.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.pepper` (macOS) or `systemctl --user restart pepper` (Linux)
