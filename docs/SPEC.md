...
│  │    • mcp__pepper__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── pepper.log               # Host stdout
│   └── pepper.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.pepper.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'pepper-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your pepper installation
...
Pepper uses a hierarchical memory system based on CLAUDE.md files.
...
Pepper has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__pepper__schedule_task]
        {
...
The `pepper` MCP server is created dynamically per agent call with the current group's context.
...
Pepper runs as a single macOS launchd service.

### Startup Sequence

When Pepper starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Pepper containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/pepper.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/pepper.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.pepper.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep pepper` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Pepper auto-starts container runtime but may fail |
...
- `logs/pepper.log` - stdout
- `logs/pepper.error.log` - stderr
