...
Pepper skills are distributed as git branches on the upstream repository. Applying a skill is a `git merge`. Updating core is a `git merge`. Everything is standard git.

This replaces the previous `skills-engine/` system (three-way file merging, `.pepper/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`qwibitai/pepper`) maintains:

- `main` — core Pepper (no skill code)
...
- `/setup`, `/debug`, `/update-pepper`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`qwibitai/pepper-skills`)
...
claude plugin install pepper-skills@pepper-skills --scope project
```
...
Pepper's `.claude/settings.json` registers the official marketplace:
...
```
qwibitai/pepper-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    pepper-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `pepper-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-pepper` — after merging main, optionally check for skill updates
...
1. Fork `qwibitai/pepper` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/pepper.git
   cd pepper
   ```
3. Run Claude Code:
   ```bash
...
Users who previously ran `git clone https://github.com/qwibitai/pepper.git` and have local customizations:

1. Fork `qwibitai/pepper` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/pepper.git
...
3. From this point, `origin` = their fork, `upstream` = qwibitai/pepper
...
In both cases:
- Delete the `.pepper/` directory (no longer needed)
...
This is the same as the existing `/update-pepper` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-pepper` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to qwibitai/pepper:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`qwibitai/pepper-skills`)
...
1. Maintains a fork of Pepper (e.g., `alice/pepper`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/pepper-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to Pepper's `.claude/settings.json`:
...
Once merged, all Pepper users automatically discover the community marketplace alongside the official one.
...
```bash
claude plugin install alice-skills@alice-pepper-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/pepper.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/pepper-skills` to discover skills from any source.
...
A flavor is a curated fork of Pepper — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "Pepper for Sales," "Pepper Minimal," "Pepper for Developers").
...
AskUserQuestion: "Start with a flavor or default Pepper?"
- Default Pepper
- Pepper for Sales — Gmail + Slack + CRM (maintained by alice)
- Pepper Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/pepper.git
...
- `upstream` — `qwibitai/pepper` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `pepper-flavor` for searchability
...
- `.pepper/` state directory

Operational skills (`setup`, `debug`, `update-pepper`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/qwibitai/Pepper.git
cd Pepper
claude
```
...
```
1. Fork qwibitai/pepper on GitHub
2. git clone https://github.com/<you>/pepper.git
3. cd pepper
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/qwibitai/pepper.git`
- Check if `origin` points to the user's fork (not qwibitai). If it points to qwibitai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install pepper-skills@pepper-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-pepper, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`qwibitai/pepper-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in Pepper. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.pepper/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.pepper/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to Pepper's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
