...
export function init(config: CloudRelayConfig): void {
  ...
  try { db.exec(`ALTER TABLE agent_events ADD COLUMN task_id TEXT;`); } catch { /* already exists */ }
  try { db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_events_task ON agent_events(task_id);`); } catch { /* already exists */ }
