...
async function pushArtifactEvent(opts: {
  ...
  const event = {
    ...
    channel: process.env.PEPPER_CHAT_JID?.startsWith('tg:') ? 'telegram' : 'unknown',
...
const IPC_DIR = process.env.PEPPER_IPC_DIR || '/workspace/ipc';
const MESSAGES_DIR = path.join(IPC_DIR, 'messages');
const TASKS_DIR = path.join(IPC_DIR, 'tasks');

// Context from environment variables (set by the agent runner)
const chatJid = process.env.PEPPER_CHAT_JID!;
const groupFolder = process.env.PEPPER_GROUP_FOLDER!;
const isMain = process.env.PEPPER_IS_MAIN === '1';
...
const tenantId = process.env.AGENT_ID;
...
// Workspace memory tool — shared knowledge across agents in a workspace
if (cloudUrl && eventSecret && tenantId) {
  const wsId = process.env.WORKSPACE_ID;
  const agentId = tenantId; // TENANT_ID = AGENT_ID in Railway

  if (wsId) {
    server.tool(
      'workspace_memory',
      `Query, search, or store shared knowledge in workspace memory. Findings stored here are visible to all agents in the workspace.

Actions:
- context: Get relevant context for a task description
- search: Search shared knowledge across all agents
- remember: Store a fact or decision for the team
- activity: View recent team activity`,
      {
        action: z.enum(['context', 'search', 'remember', 'activity']).describe('Action to perform'),
        text: z.string().optional().describe('For context: task description. For search: query. For remember: fact to store.'),
      },
      async (args) => {
        const baseUrl = `${cloudUrl}/api/workspaces/${wsId}/memory`;
        const payload = `${wsId}:${agentId}:${args.text || args.action}`;
        const sig = createHmac('sha256', eventSecret!).update(payload).digest('hex');
        ...
        try {
          ...
          switch (args.action) {
            case 'context':
              url = `${baseUrl}/context?task=${encodeURIComponent(args.text || '')}&agent_id=${agentId}`;
              break;
            case 'search':
              url = `${baseUrl}/search?q=${encodeURIComponent(args.text || '')}&agent_id=${agentId}`;
              break;
            case 'remember':
              url = `${baseUrl}/remember`;
              fetchOpts = {
                ...fetchOpts,
                method: 'POST',
                headers: { ...fetchOpts.headers as Record<string, string>, 'Content-Type': 'application/json' },
                body: JSON.stringify({ fact: args.text, agent_id: agentId }),
              };
              // Re-sign with the JSON body for POST
              const body = JSON.stringify({ fact: args.text, agent_id: agentId });
              const postSig = createHmac('sha256', eventSecret!).update(body).digest('hex');
              (fetchOpts.headers as Record<string, string>)['X-Event-Signature'] = postSig;
              break;
            case 'activity':
              url = `${baseUrl}/activity?agent_id=${agentId}`;
              break;
          }
          ...
          if (!resp.ok) {
            ...
            return { content: [{ type: 'text' as const, text: `Memory API error ${resp.status}: ${text}` }], isError: true };
          }
          const data = await resp.json();
          return { content: [{ type: 'text' as const, text: JSON.stringify(data, null, 2) }] };
        } catch (err) {
          return {
            content: [{ type: 'text' as const, text: `Workspace memory error: ${err instanceof Error ? err.message : String(err)}` }],
            isError: true,
          };
        }
      },
...
// HTML preview tool — serve interactive HTML from Railway's public URL
const railwayDomain = process.env.RAILWAY_PUBLIC_DOMAIN;
if (railwayDomain) {
  server.tool(
    'preview_html',
    `Serve an HTML file as a live preview URL the user can open in their browser.
Use this for reports, dashboards, prototypes, visualizations, or any interactive content.
The URL is live as long as the Railway instance is running.
The HTML should be self-contained (inline CSS/JS, no external dependencies).`,
    {
      file_path: z.string().describe('Absolute path to the HTML file on disk'),
      title: z.string().optional().describe('Human-readable title for the preview'),
    },
    async (args) => {
      if (!fs.existsSync(args.file_path)) {
        return {
      ...
      // Copy file to the preview directory
      const groupDir = process.env.PEPPER_WORKSPACE_GROUP || '/data/groups/webchat';
      const previewDir = path.join(groupDir, 'preview');
      fs.mkdirSync(previewDir, { recursive: true });

      const filename = path.basename(args.file_path);
      const destPath = path.join(previewDir, filename);
      fs.copyFileSync(args.file_path, destPath);

      const previewUrl = `https://${railwayDomain}/preview/${encodeURIComponent(filename)}`;

      return {
        content: [{
          type: 'text' as const,
          text: `Preview ready: ${previewUrl}\n\nShare this URL with the user — they can view "${args.title || filename}" in their browser.`,
        }],
      };
    },
  );
