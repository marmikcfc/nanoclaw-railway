...
export interface ProcessedAttachment {
  /** 'image' → Claude image content block; 'document' → Claude document block (PDFs); 'file' → generic base64 file */
...
export async function processAttachment(
  ...
  // Generic file — store as base64 for workspace file writing
  if (!filename) {
    logger.warn({ mimeType }, 'media: generic file without filename, skipping');
    return null;
  }
  return {
    type: 'file',
