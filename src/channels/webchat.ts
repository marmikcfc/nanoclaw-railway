...
const WEBCHAT_JID = 'admin@pepper';
...
export class WebchatChannel implements Channel {
  ...
  /**
   * Queue of task_ids from incoming webhooks, mirroring incomingTraceIds.
   * Each webhook optionally pushes its task_id here; processGroupMessages
  ...
  async sendMessage(_jid: string, text: string): Promise<void> {
    ...
    if (!cloudUrl || !agentId || !secret) {
      logger.error('WebchatChannel: missing env vars (PEPPER_CLOUD_URL, AGENT_ID, PEPPER_EVENT_SECRET)');
      return;
    }
