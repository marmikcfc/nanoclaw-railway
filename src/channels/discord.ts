...
export class DiscordChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.client.on(Events.MessageCreate, async (message: Message) => {
      ...
      // Handle attachments — download all non-video/audio files; video/audio get placeholders
        ...
        for (const att of message.attachments.values()) {
          ...
          if (contentType.startsWith('video/')) {
            ...
            try {
              ...
              const filename = att.name ?? undefined;
              const processed = await processAttachment(buffer, contentType, filename);
              if (processed) {
                processedAttachments.push(processed);
                this.uploadToCloud(buffer, att.name || 'file', contentType, 'discord')
                  .catch(err => logger.warn({ err, name: att.name }, 'Discord: failed to upload attachment to cloud'));
              }
              ...
              logger.warn({ err, name: att.name }, 'Discord: failed to process attachment');
              if (contentType.startsWith('image/')) {
                descriptions.push(`[Image: ${att.name || 'image'}]`);
              } else if (contentType === 'application/pdf') {
                descriptions.push(`[PDF: ${att.name || 'file.pdf'}]`);
