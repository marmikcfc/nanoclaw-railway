 ...
 *
 * Posts an HMAC-signed request to the main pepper service to trigger
 * scheduled task processing. Exits immediately after — Railway handles
 * the schedule.
 *
 * Required env vars:
 *   PEPPER_INTERNAL_URL   — e.g. http://pepper.railway.internal:3000
 *   PEPPER_EVENT_SECRET   — shared HMAC secret (same as main service)
 */
import { createHmac } from 'crypto';

const url = process.env.PEPPER_INTERNAL_URL;
const secret = process.env.PEPPER_EVENT_SECRET;

if (!url) {
  console.error('PEPPER_INTERNAL_URL is not set');
  process.exit(1);
}

if (!secret) {
  console.error('PEPPER_EVENT_SECRET is not set');
  process.exit(1);
}
