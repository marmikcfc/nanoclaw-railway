...
// --- Task context cache accessors ---

export function getTaskContextCache(
  taskId: string,
): { context: string; injected: boolean } | undefined {
  const row = db
    .prepare('SELECT context, injected FROM task_context_cache WHERE task_id = ?')
    .get(taskId) as { context: string; injected: number } | undefined;
  if (!row) return undefined;
  return { context: row.context, injected: row.injected === 1 };
}

export function setTaskContextCache(taskId: string, context: string): void {
  db.prepare(
    'INSERT OR REPLACE INTO task_context_cache (task_id, context, injected, fetched_at) VALUES (?, ?, 0, ?)',
  ).run(taskId, context, new Date().toISOString());
}

export function markTaskContextInjected(taskId: string): void {
  db.prepare('UPDATE task_context_cache SET injected = 1 WHERE task_id = ?').run(
    taskId,
  );
}

export function clearTaskContextCache(taskId?: string): void {
  if (taskId) {
    db.prepare('DELETE FROM task_context_cache WHERE task_id = ?').run(taskId);
  } else {
    db.prepare('DELETE FROM task_context_cache').run();
  }
}
