...
<dict>
    ...
    <string>${projectRoot}/logs/pepper.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/pepper.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.pepper');
  } catch {
...
/**
 * Kill any orphaned pepper node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned pepper processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/pepper.service';
    ...
    unitPath = path.join(unitDir, 'pepper.service');
...
StandardOutput=append:${projectRoot}/logs/pepper.log
StandardError=append:${projectRoot}/logs/pepper.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned pepper processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable pepper`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start pepper`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active pepper`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-pepper.sh');
  const pidFile = path.join(projectRoot, 'pepper.pid');

  const lines = [
    '#!/bin/bash',
    '# start-pepper.sh — Start Pepper without systemd',
    ...
    '    echo "Stopping existing Pepper (PID $OLD_PID)..."',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/pepper.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/pepper.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Pepper started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/pepper.log"`,
  ];
