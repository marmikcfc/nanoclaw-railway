...
describe('WebchatChannel', () => {
  ...
  beforeEach(() => {
    vi.stubEnv('PEPPER_CLOUD_URL', CLOUD_URL);
    vi.stubEnv('TENANT_ID', TENANT_ID);
    vi.stubEnv('PEPPER_EVENT_SECRET', EVENT_SECRET);
  ...
  it('ownsJid returns true only for admin@pepper', () => {
    expect(channel.ownsJid('admin@pepper')).toBe(true);
    expect(channel.ownsJid('other@pepper')).toBe(false);
  ...
  it('sendMessage POSTs webchat_agent_message to Cloud events API', async () => {
    ...
    await channel.sendMessage('admin@pepper', 'Hello from agent');
  ...
  it('sendMessage includes valid HMAC signature header', async () => {
    ...
    channel.currentTraceId = 'trace-abc-123';
    await channel.sendMessage('admin@pepper', 'Hello');
  ...
  it('sendMessage logs error and does not throw if Cloud returns error', async () => {
    ...
    await expect(channel.sendMessage('admin@pepper', 'Hi')).resolves.toBeUndefined();
  });
